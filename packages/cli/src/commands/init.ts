...
export const initCommand = new Command('init')
  ...
  .action(async (opts) => {
    ...
    try {
      // Non-interactive: just save config with provided flags
      if (nonInteractive || product) {
        if (!product) {
          console.error(chalk.red('--product is required in non-interactive mode'));
          process.exit(1);
        }
      ...
      try {
        ...
        console.error(
          chalk.red('Cannot reach Lock API. Use `lock init -y --product <slug>` to initialize manually.'),
        );
        process.exit(1);
      }
      ...
      if (!product) {
        ...
        if (product === CREATE_NEW) {
          ...
          console.log(chalk.dim(`Product "${product}" will be created on first lock commit.`));
        }
      }
      ...
      if (!feature) {
        ...
        if (feature === CREATE_NEW) {
          ...
          console.log(chalk.dim(`Feature "${feature}" will be created on first lock commit.`));
        }
      }
...
function printSuccess(product: string, feature?: string): void {
  ...
  console.log(`  ${chalk.dim('Feature:')} ${feature || '(none)'}`);
  console.log('');
